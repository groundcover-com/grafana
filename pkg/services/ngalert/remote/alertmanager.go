...
func (am *Alertmanager) PutAlerts(ctx context.Context, alerts apimodels.PostableAlerts) error {
	for _, a := range alerts.PostableAlerts {
		SanitizeWebhookLabels(a.Labels)
	}
...
// SanitizeWebhookLabels transforms alert labels to match the state history
// fingerprint: adds monitor_name from alertname, removes private labels
// (__*) and empty values.
func SanitizeWebhookLabels(labels map[string]string) {
	if alertname, ok := labels["alertname"]; ok {
		labels["monitor_name"] = alertname
	}
